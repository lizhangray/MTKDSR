...
class Trainer():
    ...
    def Train(self):
        ...
        if self.train_by_TT == True:
            ...
            teacher_1 = get_model(args.t1_model_name, args.t1_checkpoint).to(self.device)
            teacher_2 = get_model(args.t2_model_name, args.t2_checkpoint).to(self.device)

        l1_criterion = nn.L1Loss().to(self.device)              # MAE，用于计算L1损失
        if args.Rate_lpts > 0:
            vgg19 = get_Extractor().eval().to(self.device)      # 用于计算感知损失
        if args.Rate_lm > 0:
            frac = fraction_differential(v=0.6, althea=0.3)     # 边缘算子， 用于计算边缘损失

        for epoch in range(args.start_epoch, args.nEpochs + 1): # 1 - nEpoches
            ...
            lr_scheduler = torch.optim.lr_scheduler.StepLR(self.optimizer, step_size=args.lr_step_size, gamma=args.lr_gamma)

            for iteration, (lr_tensor, hr_tensor) in enumerate(self.train_loader, 1):

                self.model.train()
                self.optimizer.zero_grad()

                lr_tensor = lr_tensor.to(self.device)  # ranges from [0, 1]
                sr_pred = self.model(lr_tensor)

                if self.train_by_TT == True:
                    # TODO:
                    t1_pred = teacher_1(lr_tensor)
                    t2_pred = hr_tensor.to(self.device)
                    #　t2_pred = teacher_2(lr_tensor)
                else:
                    t1_pred = t2_pred = hr_tensor.to(self.device)

                loss_l1 = l1_criterion(sr_pred, t1_pred)
                loss_sr = args.Rate_l1 * loss_l1

                # loss_pts = l1(VGG(t2_pred, sr_pred))
                if args.Rate_lpts > 0:
                    loss_p_t_s = (l1_criterion(vgg19(sr_pred), vgg19(t2_pred)))
                    loss_sr += args.Rate_lpts * loss_p_t_s

                # loss_m = l1(laplace(t3_pred, sr_pred))
                if args.Rate_lm > 0:
                    Y_esr = frac.Laplace(sr_pred.cpu().data)
                    Y_ehr = frac.Laplace(t1_pred.cpu().data)
                    if self.cuda:
                        Y_esr = torch.from_numpy(Y_esr).float().cuda()
                        Y_ehr = torch.from_numpy(Y_ehr).float().cuda()
                    else:
                        Y_esr = torch.from_numpy(Y_esr).float()
                        Y_ehr = torch.from_numpy(Y_ehr).float()
                    loss_m = l1_criterion(Y_esr, Y_ehr)
                    loss_sr += args.Rate_lm * loss_m
                ...
                if iteration % 100 == 0:
                    ...
                    print("\r===> Training: Epoch[{}/{}]({}/{})\t".format(self.epoch, self.args.nEpochs, iteration, len(self.train_loader)), end="")
                    print("Loss:{:.3f} = L1:{:.3f}".format(loss_sr.item(), loss_l1.item()), end="")
                    if args.Rate_lm > 0:
                        print(" + Lm:{:.3f}".format((args.Rate_lm * loss_m).item()), end="")
                    if args.Rate_lpts > 0:
                        print(" + Lpts:{:.3f}".format((args.Rate_lpts * loss_p_t_s).item()), end="")
                    print("\tLearn_rate: {:.8f}\t".format(self.optimizer.param_groups[0]['lr']), end="")

                    with open(self.train_loss_log_path, 'a') as self.loss_log:
                        print("Loss:{:.3f} = L1:{:.3f}".format(loss_sr.item(), loss_l1.item()), file=self.loss_log, end="")
                        if args.Rate_lm > 0:
                            print(" + Lm:{:.3f}".format((args.Rate_lm * loss_m).item()),file=self.loss_log,  end="")
                        if args.Rate_lpts > 0:
                            print(" + Lpts:{:.3f}".format((args.Rate_lpts * loss_p_t_s).item()),file=self.loss_log,  end="")
                        print("", file=self.loss_log)
